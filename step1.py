#!/usr/bin/env python
# python gtexfix/step1.py tex_files/
#-----------------------------------------
...
# file per salvare tag e codice vero
jsonFile = r"tags.json"
print("JSON file in "+jsonFile)
jsonData = {}
if(isfile(jsonFile)):
    with open(jsonFile, 'r') as source_file:
        jsonData = json.load(source_file.read()) """

LIMIT = 5000

parser = argparse.ArgumentParser()
parser.add_argument('filename')
parser.add_argument("-i","--ignorelimits", action='store_true', help="No split - Ignore LIMIT") # action='store_true' = usually false
args = parser.parse_args()

if (args.ignorelimits):
    LIMIT = 2e12 # ITA pd
...
conflicts=re.findall('\[ *[012][\.][0-9]+\]',source) # tolto la virgola, non credo servirà! conflicts=re.findall('\[ *[012][\.\,][0-9]+\]',source)
if(conflicts!=[]):
...
with open('gtexfix_comments.json', 'w', encoding="utf8") as fp:
...
with open('gtexfix_latex.json', 'w', encoding="utf8") as fp:
    json.dump(latex, fp)

### Replace LaTeX commands, formulas and comments by tokens - better in two steps: issues when 1 into another (e.g. \\figPro{2R1Celement}{Electical model of heat transfer from layer $l_{-1}$} breaks after '-1}') then first all $$..$$ and $...$
...
recommand = re.compile(r'(\$+)(?:(?!\1)[\s\S])*\1')
commands={}
cont = 0
for m in recommand.finditer(text):
    commands['[3.{}]'.format(str(cont))]=(m.group())
    cont+=1
nc=0
def repl_math(obj):
    global nc
    nc += 1
    return '[3.%d]'%(nc-1)
text=recommand.sub(repl_math,text)
...
# commands={} otherwise overwrite
cont = 0
for m in recommand.finditer(text):
    commands['[2.{}]'.format(str(cont))]=(m.group())
    cont+=1
nc=0
def repl_f(obj):
    global nc
...
with open('gtexfix_commands.json', 'w', encoding="utf8") as fp:
    json.dump(commands, fp)

### Save the processed output to .txt file
limit=LIMIT # Estimated Google Translate character limit
...
for m in re.finditer(r'\.\n',text):
    if(m.end()-start<limit):
        ...
        output_filename = filebase+'_%02d.txt'%npart
        npart+=1
